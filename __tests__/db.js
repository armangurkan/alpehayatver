...
const db = require('../server/models/db/markets.js');
...
describe('db unit tests', () => {
  ...
  describe('#sync', () => {
    ...
    // Finish unit testing the sync function

    it('overwrites previously existing markets', () => {
      const marketList = [{ location: 'here', cards: 10 }, { location: 'there', cards: 0 }];
      const result = db.sync(marketList);
      expect(result).not.toBeInstanceOf(Error);
      const table = JSON.parse(fs.readFileSync(testJsonFile));
      expect(table).toEqual(marketList);
    ...
    /**
     *  Type validation is not yet correctly implemented! Follow the TDD
    ...
    it('returns an error when location value is not a string', () => {
      const marketList = [{ location: 4, cards: 10 }, { location: 'there', cards: 0 }];
      const result = db.sync(marketList);
      expect(result).toBeInstanceOf(Error);
    });
  ...
  // Extension Unit test the #find and #drop functions
  describe('#find', () => {
