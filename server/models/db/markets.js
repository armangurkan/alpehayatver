...
db.sync = (markets) => {
  ...
  if (markets.some(m => typeof m.location !== 'string')) {
    return new Error('the type of the location field is not a string, it should be a string');
  }
  if (markets.some(m => typeof m.cards !== 'number')) {
    return new Error('the type of the cards field is not a number, it should be a number');
  }
  db.write(markets);
  db.reset();
  return marketList;
 ...
 * @return {Array} the list of markets
 */
db.find = () => JSON.parse(fs.readFileSync(writeLocation));
